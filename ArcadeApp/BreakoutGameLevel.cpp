...
void BreakoutGameLevel::Update(uint32_t dt, Ball& ball) {
	...
	for (auto& block : _blocks) {
		...
		if (!block.IsDestroyed() && block.HasCollided(ball.GetBoundRect(), edge)) {
			collidedBlocks.push_back(block);

			float mag = block.GetCollisionOffset(ball.GetBoundRect()).Mag();
	...
	for (const auto& block : collidedBlocks) {
		BoundaryEdge edge;
		if (block.HasCollided(ball.GetBoundRect(), edge)) {
			Vec2D p;
...
void BreakoutGameLevel::Draw(Screen& screen) {
	for (auto& block : _blocks) {
		if (!block.IsDestroyed()) block.Draw(screen);
	}
}
...
LayoutBlock FindLayoutBlockForSymbol(const std::vector<LayoutBlock>& blocks, char symbol) {
	for (size_t i = 0; i < blocks.size(); ++i) {
		if (blocks[i].symbol == symbol) return blocks[i];
	}
	return LayoutBlock();
}

bool BreakoutGameLevel::IsLevelComplet() const {
	for (size_t i = 0; i < _blocks.size(); ++i) {
		if (!_blocks[i].IsDestroyed() && _blocks[i].GetHP() != Block::UNBREAKABLE) return false;
	}
	return true;
}
