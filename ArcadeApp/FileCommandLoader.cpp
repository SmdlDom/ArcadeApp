...
bool FileCommandLoader::LoadFile(const std::string& filePath) {
	...
	while (!inFile.eof()) {
		...
		if ((commandPos = line.find(":")) != std::string::npos) {
			...
			for (size_t commandIndex = 0; commandIndex < _commands.size(); ++commandIndex) {
				if (commandStr == _commands[commandIndex].command) {
					if (_commands[commandIndex].commandType == COMMAND_ONE_LINE) {
						ParseFuncParams params;
						params.dilimitPos = dilimitPos;
						params.lineNum = 0;
						params.line = line;
						_commands[commandIndex].parseFunc(params);
					} else {
